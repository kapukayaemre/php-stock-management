...
class stok extends controller
{
    ...
    public function update($id){
        ...
        if($_POST) {
            ...
            if($adet != 0 && $fiyat != "") {
                $insert = $this->model('stokModel')->update($id,$urun_id,$musteri_id,$islem_tipi,$adet,$fiyat);
                if($insert){
                    helper::flashData('statu', 'Stok Başarıyla Güncellendi');
                    helper::redirect(SITE_URL. '/stok');
                } else {
                    helper::flashData('statu', 'Stok Eklenemedi!!');
    ...
    public function delete($id){
        $query = $this->model('stokModel')->delete($id);
        helper::redirect(SITE_URL.'/stok');
    }
